...
3. Either:
   - Edit `cloudPeerJsConfig` in `public/config.js` to point to your host, or
   - Use URL overrides (no code changes):

```text
?peerHost=your-domain.com&peerPort=443&peerPath=/myapp&peerSecure=true
```

Both callers must use the same signaling host/path settings.
